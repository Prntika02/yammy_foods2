...
if(empty($email)){
    ...
    $id = $_SESSION['auth']['id'];
    $query = "SELECT email FROM user1 WHERE email = '$email' AND id != '$id'";
...
if(count($errors) > 0){
    ...
    if(!file_exists(UPLOADS_PATH)){
        ...
        move_uploaded_file($profileImage['tmp_name'], UPLOADS_PATH."/$fileName");
         ...
         $query = "UPDATE user1 SET name='$name',email='$email',profile_img='uploads/$fileName' WHERE id = '$userId'";
    
    } else{
        $query = "UPDATE user1 SET name='$name',email='$email'  WHERE id = '$userId'";
